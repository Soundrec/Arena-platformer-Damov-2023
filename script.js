...
colors[2][1] = 'rgba(100,145,150,';
colors[2][2] = 'rgba(255,255,255,';
...
Firefly.prototype.walk = function() {
	...
	var deltaY = Math.abs(mouseY - this.y);
	var deltaX = Math.abs(mouseX - this.x);
	
	if (deltaY < this.width*15 && deltaX < this.width*15)  {
	...
	if (this.speed > this.width*2) {
		this.speed = this.speed-1;
	}
	
	if (this.speed < (this.width <= 10) ? 2 : 1) {
		this.speed = 1;
...
function logon( uname){
	const thing = document.getElementById("overlay");
	thing.innerHTML = "<p class=\"uname\">" + uname + "  </p>"
}
...
async  function submitAuth(){
    ...
    const name = form.elements['aname'].value;
    const password1 = form.elements['apwd'].value;
	...
	if (jsonData["Good"] != undefined) logon(jsonData["Good"]);
}
...
async  function submitRegistration(){
    ...
    const password1 = form.elements['rpwd1'].value;
	const password2 = form.elements['rpwd2'].value;
	...
	const jsonData = await response.json();
	if (jsonData["Good"] != "") logon(jsonData["Good"]);
}

async function reauth(){
	const response = await  fetch('/api/relog.php');
	const jsonData = await response.json();
	if (jsonData["Good"] != undefined) logon(jsonData["Good"]);
}	